use ITI_Mongo
//Q1 1.	Create a Database named "ITI_Mongo".
...
//Q2:Create a Collection named "Staff" + Q3:Insert one document into the "Staff" collection: {_id, name, age, gender, department}.
db.Staff.insertOne({
   "_id":1,
   "name":"Aya",
   "age":23,
...
//db test insertmany(db.staff.find().toarray())
db.test.insertMany(
[
   {
      "_id":2,
...
//Q9.a-a. With justification, explain which document will be deleted if more than one has age = 15. (Try it.) 

//Answer:MongoDB deletes only one document that matches the condition. If more than one document has age = 15, 
//MongoDB deletes the first matching document

//Q9.b- First insert: db.collection.insertOne({ _id: 5, name: "ahmed", age: 15 })
db.test.insertOne(
     {
      "_id":5,
      "name":"Ahmed",
...
//Q9.c-Second insert: db.collection.insertOne({ _id: 6, name: "eman", age: 15 }) 
db.test.insertOne(
     {
      "_id":6,
      "name":"Eman",
...
//Q9.d.  When you run deleteOne, will it delete ahmed or eman?
//Answer:Delete first match
db.test.deleteOne({ age: 15 })
db.test.find({})
